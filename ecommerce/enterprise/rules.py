...
@rules.predicate
def is_enterprise_coupon_admin(user, obj):
    """
    Returns whether the user is an an enterprise admin.
    """
    ecommerce_feature_role_assignments = EcommerceFeatureRoleAssignment.objects.filter(
        user=user, role=ENTERPRISE_COUPON_ADMIN_ROLE
    )
    if not user.groups.filter(name='enterprise_admin').exists() or not ecommerce_feature_role_assignments.exists():
        return False
...
rules.add_perm('enterprise.can_view_coupon', is_enterprise_coupon_admin | rules.predicates.is_staff)
rules.add_perm('enterprise.can_assign_coupon', is_enterprise_coupon_admin | rules.predicates.is_staff)
